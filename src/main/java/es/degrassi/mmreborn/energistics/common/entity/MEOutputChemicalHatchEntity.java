...
public class MEOutputChemicalHatchEntity extends MEEntity {
  ...
  public MEOutputChemicalHatchEntity(BlockPos pos, BlockState blockState, MEHatchSize size) {
  ...
  public BasicChemicalTank copyTank() {
  ...
  protected BasicChemicalTank createInventory() {
    return new AEChemicalTankHolder(this);
  }
  ...
  public void openMenu(Player player, MenuHostLocator locator) {
    if (!getLevel().isClientSide()) {
      if (getSize().isAdvanced())
        MenuOpener.open(MEOutputChemicalHatchContainer.ADVANCED_TYPE, player, locator);
      else
        MenuOpener.open(MEOutputChemicalHatchContainer.TYPE, player, locator);
    }
  }
  ...
  public void returnToMainMenu(Player player, ISubMenu subMenu) {
    if (getSize().isAdvanced())
      MenuOpener.open(MEOutputChemicalHatchContainer.ADVANCED_TYPE, player, subMenu.getLocator());
    else
      MenuOpener.open(MEOutputChemicalHatchContainer.TYPE, player, subMenu.getLocator());
  }

  private class RequestActionSource extends MachineSource {
    private final RequestContext context;

    RequestActionSource(final MEOutputChemicalHatchEntity machine, IActionHost host) {
      super(host);
  ...
  private class RequestContext {
    ...
    public int getPriority() {
      return MEOutputChemicalHatchEntity.this.getPriority();
    }
  }
  ...
  private class Ticker implements IGridTickable {
    ...
    public TickingRequest getTickingRequest(IGridNode node) {
      return new TickingRequest(TickRates.Interface, !MEOutputChemicalHatchEntity.this.hasWorkToDo());
    }

    public TickRateModulation tickingRequest(IGridNode node, int ticksSinceLastCall) {
      if (!MEOutputChemicalHatchEntity.this.getMainNode().isActive()) {
        return TickRateModulation.SLEEP;
      } else {
        boolean couldDoWork = MEOutputChemicalHatchEntity.this.updateStorage();
        return MEOutputChemicalHatchEntity.this.hasWorkToDo() ? (couldDoWork ? TickRateModulation.URGENT : TickRateModulation.SLOWER) : TickRateModulation.SLEEP;
      }
    }
  }
}
