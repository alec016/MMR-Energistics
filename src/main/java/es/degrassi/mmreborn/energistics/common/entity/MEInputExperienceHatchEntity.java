...
@Getter
public class MEInputExperienceHatchEntity extends MEEntity {
  private final KeyStorage internalBuffer; // Do not use KeyCounter, use our simple implementation
  private final BasicExperienceTank inventory;
  ...
  private void updatePlan(int slot) {
    ...
    if (stored != null) {
      this.plannedWork[slot] = new GenericStack(ExperienceKey.KEY,
          storage.getCapacity(ExperienceKeyType.TYPE) - stored.amount());
    } else {
  ...
  private class RequestActionSource extends MachineSource {
    private final MEInputExperienceHatchEntity.RequestContext context;

    RequestActionSource(final MEInputExperienceHatchEntity machine, IActionHost host) {
      super(host);
      this.context = machine.new RequestContext();
    }

    public <T> Optional<T> context(Class<T> key) {
      return key == MEInputExperienceHatchEntity.RequestContext.class ? Optional.of(key.cast(this.context)) : super.context(key);
    }
  }
  ...
  private class RequestContext {
    ...
    public int getPriority() {
      return MEInputExperienceHatchEntity.this.getPriority();
    }
  }
  ...
  private class Ticker implements IGridTickable {
    ...
    public TickingRequest getTickingRequest(IGridNode node) {
      return new TickingRequest(TickRates.Interface, !MEInputExperienceHatchEntity.this.hasWorkToDo());
    }

    public TickRateModulation tickingRequest(IGridNode node, int ticksSinceLastCall) {
      if (!MEInputExperienceHatchEntity.this.getMainNode().isActive()) {
        return TickRateModulation.SLEEP;
      } else {
        boolean couldDoWork = MEInputExperienceHatchEntity.this.updateStorage();
        return MEInputExperienceHatchEntity.this.hasWorkToDo() ? (couldDoWork ? TickRateModulation.URGENT : TickRateModulation.SLOWER) : TickRateModulation.SLEEP;
      }
    }
  }
}
