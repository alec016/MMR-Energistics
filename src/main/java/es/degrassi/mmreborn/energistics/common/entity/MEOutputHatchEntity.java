...
public class MEOutputHatchEntity extends MEEntity {
  ...
  public MEOutputHatchEntity(BlockPos pos, BlockState blockState, MEHatchSize size) {
  ...
  public void openMenu(Player player, MenuHostLocator locator) {
    if (!getLevel().isClientSide()) {
      if (getSize().isAdvanced())
        MenuOpener.open(MEOutputHatchContainer.ADVANCED_TYPE, player, locator);
      else
        MenuOpener.open(MEOutputHatchContainer.TYPE, player, locator);
    }
  }
  ...
  public void returnToMainMenu(Player player, ISubMenu subMenu) {
    if (getSize().isAdvanced())
      MenuOpener.open(MEOutputHatchContainer.ADVANCED_TYPE, player, subMenu.getLocator());
    else
      MenuOpener.open(MEOutputHatchContainer.TYPE, player, subMenu.getLocator());
  }

  private class RequestActionSource extends MachineSource {
    private final RequestContext context;

    RequestActionSource(final MEOutputHatchEntity machine, IActionHost host) {
      super(host);
  ...
  private class Ticker implements IGridTickable {
    ...
    public TickRateModulation tickingRequest(IGridNode node, int ticksSinceLastCall) {
      if (!MEOutputHatchEntity.this.getMainNode().isActive()) {
        ...
        boolean couldDoWork = MEOutputHatchEntity.this.updateStorage();
        return MEOutputHatchEntity.this.hasWorkToDo() ? (couldDoWork ? TickRateModulation.URGENT : TickRateModulation.SLOWER) : TickRateModulation.SLEEP;
      }
    }
  }
}
